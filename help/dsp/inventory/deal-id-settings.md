...
|  | [!UICONTROL Billing method] | 모든 거래 ID는 [!DNL Adobe]-자금 및 송장 발행 Advertising Cloud은 사용을 기반으로 사용 가능한 모든 미디어 공급업체에 지불하고, 공급업체와의 불일치를 관리하고, 하나의 통합 송장을 계정에 보냅니다. 이 옵션은 계정 환율 카드에 설명된 대로 추가 비용을 발생시킵니다. | 예 | 아니요 |
