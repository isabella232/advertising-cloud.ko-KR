...
| [!UICONTROL Viewability IAS] | [!DNL IAS]의 내역 데이터를 사용하여 필요한 최소 보기 가능 비율을 설정합니다. 지정된 임계값이 충족되면 노출에 입찰합니다. | 이 필터는 캠페인 수준 [!DNL IAS] 통합을 통해 더 많은 데이터를 가져오므로 가장 잘 작동합니다.<br><br>캠페인이 데이터를 사용하도록 구성된  [!DNL IAS] 경우 가장 좋은 방법은 패키지 최적화 목표 &quot;[!UICONTROL Lowest vCPM (IAS)]&quot;와 함께 이 필터를 사용하는 것입니다. 통합이 활성화되지 않은 경우 최적화 목표 &quot;[!UICONTROL Lowest CPM]&quot;와 함께 이 필터를 사용하십시오. |
| [!UICONTROL Viewability Moat] | [!DNL Moat]의 내역 데이터를 사용하여 필요한 최소 보기 가능 비율을 설정합니다. 지정된 임계값이 충족되면 노출에 입찰합니다. | 이 필터는 캠페인 수준 [!DNL Moat] 통합을 통해 더 많은 데이터를 가져오므로 가장 잘 작동합니다.<br><br>캠페인이 데이터를 사용하도록  [!DNL Moat] 구성된 경우 패키지 최적화 목표 &quot;[!UICONTROL Lowest vCPM (Moat)]&quot;를 사용하는 것이 좋습니다. 통합이 활성화되지 않은 경우 최적화 목표 &quot;[!UICONTROL Lowest CPM]&quot;와 함께 이 필터를 사용하십시오. |
